...
export const EditorCanvas = ({ children, activeTool }: { children: React.ReactNode, activeTool?: WhiteboardTool }) => {
    ...
    const handleCanvasClick = (e: React.MouseEvent<HTMLDivElement>) => {
        ...
        // Check if ROOT node exists before adding
        const rootNode = query.node("ROOT").get();
        if (!rootNode) {
            console.warn('ROOT node not found, cannot add element');
            return;
        }
        ...
        if (nodeToCreate) {
            ...
            // Dispatch a custom event to notify page.tsx to reset tool to select
            window.dispatchEvent(new CustomEvent('reset-active-tool'));
        }
    };
