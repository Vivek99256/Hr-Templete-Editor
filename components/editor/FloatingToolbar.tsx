            ...
            <div className="bg-white/90 backdrop-blur-xl rounded-[28px] shadow-[0_20px_40px_-15px_rgba(0,0,0,0.1)] border border-white max-w-min ring-1 ring-black/[0.03] p-2 flex flex-col gap-2 relative z-10">
                    ...
                    {hoveredTool === 'draw' && (
                        <div className="absolute left-full top-0 pl-2 h-full z-50">
                            <div className="bg-white/95 backdrop-blur-xl rounded-2xl shadow-[0_20px_40px_-15px_rgba(0,0,0,0.15)] border border-white/50 ring-1 ring-black/[0.03] p-2 flex flex-col gap-1 min-w-[130px] animate-in slide-in-from-left-2 fade-in duration-200">
                                <button onClick={() => setActiveTool('draw_pen')} className={`p-2 rounded-xl flex items-center gap-3 hover:bg-neutral-50 transition-colors ${activeTool === 'draw_pen' ? 'bg-sky-50 text-sky-600' : 'text-neutral-700'}`}>
                                    <Pen className="w-4 h-4 text-slate-800" /> <span className="text-xs font-semibold">Pen</span>
                                </button>
                                <button onClick={() => setActiveTool('draw_marker')} className={`p-2 rounded-xl flex items-center gap-3 hover:bg-neutral-50 transition-colors ${activeTool === 'draw_marker' ? 'bg-sky-50 text-sky-600' : 'text-neutral-700'}`}>
                                    <PenLine className="w-4 h-4 text-red-500" /> <span className="text-xs font-semibold">Marker</span>
                                </button>
                                <button onClick={() => setActiveTool('draw_highlighter')} className={`p-2 rounded-xl flex items-center gap-3 hover:bg-neutral-50 transition-colors ${activeTool === 'draw_highlighter' ? 'bg-sky-50 text-sky-600' : 'text-neutral-700'}`}>
                                ...
                                <button onClick={() => setActiveTool('draw_eraser')} className={`p-2 rounded-xl flex items-center gap-3 hover:bg-red-50 hover:text-red-600 transition-colors ${activeTool === 'draw_eraser' ? 'bg-red-50 text-red-600' : 'text-neutral-700'}`}>
                ...
                >
                    ...
                    {hoveredTool === 'line' && (
                        <div className="absolute left-full top-0 pl-2 h-full z-50">
                            <div className="bg-white/95 backdrop-blur-xl rounded-2xl shadow-[0_20px_40px_-15px_rgba(0,0,0,0.15)] border border-white/50 ring-1 ring-black/[0.03] p-2 flex flex-col gap-1 min-w-[130px] animate-in slide-in-from-left-2 fade-in duration-200">
                                <button onClick={() => setActiveTool('line_straight')} className={`p-2 rounded-xl flex items-center gap-3 hover:bg-neutral-50 transition-colors ${activeTool === 'line_straight' ? 'bg-sky-50 text-sky-600' : 'text-neutral-700'}`}>
                                    <StraightLineIcon className="w-4 h-4 text-slate-800" /> <span className="text-xs font-semibold">Straight</span>
                                </button>
                                <button onClick={() => setActiveTool('line_curved')} className={`p-2 rounded-xl flex items-center gap-3 hover:bg-neutral-50 transition-colors ${activeTool === 'line_curved' ? 'bg-sky-50 text-sky-600' : 'text-neutral-700'}`}>
                                    <CurvedLineIcon className="w-4 h-4 text-slate-800" /> <span className="text-xs font-semibold">Curved</span>
                                </button>
                                <button onClick={() => setActiveTool('line_elbow')} className={`p-2 rounded-xl flex items-center gap-3 hover:bg-neutral-50 transition-colors ${activeTool === 'line_elbow' ? 'bg-sky-50 text-sky-600' : 'text-neutral-700'}`}>
