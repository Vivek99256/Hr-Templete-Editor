...
export const DrawingOverlay = ({ activeTool }: { activeTool: WhiteboardTool }) => {
    ...
    const addNodeToRoot = (nodeToCreate: any) => {
        ...
        const rootNode = query.node(parentId).get();
        if (!rootNode) {
            console.warn('Parent node not found, cannot add element');
            return false;
        }
        ...
        if (activeTool.startsWith('line_')) {
            ...
            const nodeToCreate = query.parseReactElement(
                <LineBlock
                    lineType={lineType}
                    x={startNode.x}
                    y={startNode.y - 30} // Shift by half-height so line is centered on cursor drag path
                    width={Math.max(20, distance)} // Prevent 0-width collapse
                    height={60}
                    rotation={angle}
                />
            ).toNodeTree();

            if (addNodeToRoot(nodeToCreate)) {
                window.dispatchEvent(new CustomEvent('reset-active-tool')); // auto-reset to select after drawing a line
            }
        } else {
            // Drawing Mode
